...
(function($) {
    $.tracky = function(container, options) {
        var plugin = this, $container = $(container), $app = false, name = "Tracky", slug = "tracky", title = name + " jQuery Plugin", logging = true, logPrefix = name, delay = 2500, defaults = {
            delay: delay,
            Swiper: {
                direction: "horizontal",
                loop: false,
            ...
            App: {
                ...
                get Filters() {
                    return '<div class="' + classes.App.Sections.Filter + '"><ul class="' + classes.List.Horizontal + " " + classes.List.Pill + '"><li><label class=""><input type="checkbox" name="' + name.Filter + '" value="1" id="' + ids.Filters.Remaining + '" ' + (player_info.FilterRemaining == 1 ? "checked" : "") + " > Show remaining only </label></li></ul></div>";
                },
                        ...
                        for (var i = 0; i < groupCraftables.length; i++) {
                            ...
                            for (var s = 0; s < craftableCollectables.length; s++) {
                                ...
                                for (var q = 0; q < quantity; q++) {
                                    var isCollected = q < quantityCollected, availableVal = isCollected ? classes.Flag.Available : "", isSubmitted = q < quantitySubmitted, checkedVal = isSubmitted ? property.Checked : "", checkedClass = isSubmitted ? classes.Flag.Checked : "", disabledVal = isSubmitted && !isCollected ? classes.Flag.Disabled : "";
                            ...
                            var craftedCraftable = player_game_crafted.filter(function(obj) {
                                return obj.Craftable == groupCraftables[i].ID;
                            })[0], totalRemaining = totalCollectable - totalSubmitted, isSubmittable = totalSubmitted < totalCollectable, isCraftable = !isSubmittable, isCrafted = typeof craftedCraftable !== "undefined" && craftedCraftable.Acquired == 1, abled = isSubmittable ? property.Disabled : "", opened = isSubmittable ? property.Open : "", available = isCraftable ? classes.Flag.Available : "", checkedVal = isCrafted ? property.Checked : "", craftableClass = totalRemaining == 0 ? classes.Flag.Disabled : "", checkedClass = isCrafted ? classes.Flag.Checked : "";
        ...
        var _updateCraftable = function(data) {
            ...
            if (typeof state.Interval.Update[craftable] !== "undefined" && state.Interval.Update[craftable] !== false) clearInterval(state.Interval.Update[craftable]);
            var action = "insert";
        ...
        var _collectCollectable = function(data) {
            ...
            var collectable = data.collectable, checkedSelector = "." + classes.Collectable.Input + "[value=" + collectable + "]:" + property.Checked, checked = $(checkedSelector), quantity = checked.length, game_id = $("#" + ids.Game).val(), collectableCurrent = player_game_collected.filter(function(obj) {
        ...
        var _updateFilter = function(data) {
            ...
            if (typeof state.Interval.Update[player_id] !== "undefined" && state.Interval.Update[player_id] !== false) clearInterval(state.Interval.Update[player_id]);
            var data = {
