...
// RDR2
// MERCHANT REPORT - SPEND BY PRODUCT CATEGORY
// ADMIN
// VIEW
...
if (isset($merchant_report_data) && !empty($merchant_report_data)) : 
	...
	$reports_export_link = $_SERVER["PHP_SELF"] . "?page=rdr2-merchants&action=export-spreadsheet&data=merchant-reports&report=spend_by_product&id=" . $id;

?>
<div class="admin-graph"><canvas id="ProductSpendChartContainer"></canvas></div>
...
(function($) {
	...
	var config2 = {
		type: 'pie',
	    data: {
	        labels: [ <?php foreach ($merchant_report_data as $data) echo "'" . $data["ProductCode"] . " $', "; ?> ],
	        datasets: [{
	...
	$(document).ready(function() {
		...
		var ProductChart = new Chart($("#ProductSpendChartContainer"), config2);
	});
