...
if (isset($merchant_report_data) && !empty($merchant_report_data)) : 
	...
	$graph_data = array_slice($merchant_report_data, 0, 5);
...
?>
<div class="admin-graph"><canvas id="PostcodeAnalysisChartContainer"></canvas></div>
...
(function($) {
	...
	var config2 = {
		type: 'pie',
	    data: {
	        labels: [ <?php foreach ($graph_data as $data) echo "'Postcode " . $data["Postcode"] . "', "; ?> ],
	        datasets: [{
	            data: [ <?php foreach ($graph_data as $i => $data) : ?>'<?php echo $data["DollarsSpent"]; ?>', <?php endforeach?> ],
	            backgroundColor: [
	    ...
	    options: {
	    	title: {
				text: 'Analysis by Postcode - Top 5',
				display: true,
				fontSize: 16,
	...
	$(document).ready(function() {		
		var AgeChart = new Chart($("#PostcodeAnalysisChartContainer"), config2);
	});
